...
namespace MCDawn
{
    public class Group
    {
        ...
        public int reqHours; // 0 for no time ranking, this is how many hours you must have to promote yourself, not the how much hours required for the next rank.
        ...
        public Group(LevelPermission Perm, int maxB, string fullName, char newColor, string file, int trankhours = 1) {
            Permission = Perm;
            maxBlocks = maxB;
            trueName = fullName;
        ...
        public static void InitAll()
        {
            ...
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Banned) == null) GroupList.Add(new Group(LevelPermission.Banned, 1, "Banned", '8', "banned.txt", 0));
            if (Server.useGriefer) { if (GroupList.Find(grp => grp.Permission == LevelPermission.Griefer) == null) GroupList.Add(new Group(LevelPermission.Griefer, 1, "Griefer", 'f', "griefer.txt", 0)); }
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Guest) == null) GroupList.Add(new Group(LevelPermission.Guest, 1, "Guest", '7', "guest.txt", 0));
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Builder) == null) GroupList.Add(new Group(LevelPermission.Builder, 400, "Builder", '2', "builders.txt", 0));
            if (GroupList.Find(grp => grp.Permission == LevelPermission.AdvBuilder) == null) GroupList.Add(new Group(LevelPermission.AdvBuilder, 1200, "AdvBuilder", '3', "advbuilders.txt", 0));
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Operator) == null) GroupList.Add(new Group(LevelPermission.Operator, 2500, "Operator", 'c', "operators.txt", 0));
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Admin) == null) GroupList.Add(new Group(LevelPermission.Admin, 65536, "SuperOP", 'e', "uberOps.txt", 0));
        ...
        public static void saveGroups(List<Group> givenList)
        {
            ...
            SW.WriteLine("#ReqHours = num");
            SW.WriteLine("#     This determines how many hours the rank needs to rank up via Time Ranking.");
            SW.WriteLine("#     Put this number at 0 to disable time ranking for this rank.");
            SW.WriteLine();
            SW.WriteLine();
                ...
                if (grp.name != "nobody")
                {
                    ...
                    foreach (string s in fns)
                        if (s == grp.fileName)
                        {
                            count++;
                            if (count > 1)
                                grp.fileName = "_" + grp.fileName;
                        } // prevent filename collision
                    SW.WriteLine("FileName = " + grp.fileName);
                    SW.WriteLine("ReqHours = " + grp.reqHours);
                    SW.WriteLine();
                }
            }
        ...
        public static Group Find(string name)
        {
            ...
            if (name == "super") name = "superop"; // Removed "admin" so Admin can be used as a rank name.
    ...
    public class GrpCommands
    {
        ...
        public static void fillRanks()
        {
            ...
            catch (Exception e) { Server.ErrorLog(e); Server.s.Log("Command permission loading failed. See error log."); return; }
        }
